...
export function SignalInput({ value, onValueChange, onGenerate, loading, error }: SignalInputProps) {
  ...
  return (
    <form className="signal-form" onSubmit={submit}>
      ...
      <textarea id="signal" value={value} maxLength={280} onChange={(e) => onValueChange(e.target.value)} placeholder="오늘의 감정이나 문장을 입력하세요." aria-describedby="signal-message signal-count" disabled={loading} />
